...
import com.vocketlist.android.view.EmptyRecyclerView;
import com.vocketlist.android.view.EmptyView;
 ...
 * @since 2017. 2. 13.
 */
public class MyListActivity extends DepthBaseActivity implements RecyclerViewItemClickListener {
    ...
    private void resList(BaseResponse<MyListModel> response) {
        if(response != null && response.mResult != null && response.mResult.mMyListList != null) {
            mAdapter = new MyListAdapter(response.mResult.mMyListList, this);
        }
        else mAdapter = new MyListAdapter(new ArrayList<>(), this);
    ...
    private void reqAdd(int year, String content) {
        ...
        MyListServiceManager.write(year, content, false)
                .observeOn(AndroidSchedulers.mainThread())
                .doOnTerminate(() -> hideProgressDialog(true))
                .subscribe(new EmptySubscriber<Response<BaseResponse<MyListModel.MyList>>>() {
                    ...
                    public void onNext(Response<BaseResponse<MyListModel.MyList>> baseResponseResponse) {
                        Log.d(TAG, baseResponseResponse.toString());
